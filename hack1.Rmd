...
library(class)
```
2. Define your data exploration, imputation and visualization approach <br>
...
6. Build 3 Models, each using one of different type of algorithm. Send me the model building command.<br>
=> model1:<br>
   hd.model1<-tree(com_level~.,data=hd_train)<br>
   <br>
   model2:<br>
   hd_pred_label= knn(train=hd_train1,test=hd_test1,cl=hd_train_label,3)<br>
   <br>
   model3:<br>
   model3=naiveBayes(com_level~.,data=hd_train)<br>
   <br>
   When different groups of features are selected the accuracy rate is 82% and below whereas when     all features are included the accuracy rate is 84%<br>
   <br>
   
7. Predict your model performance on each of the 3 models and submit<br>
=>
...
8. Generalization<br>
=> The maximum accuracy rate is 84% and minimum accuracy rate is 76% considering the three models which is between underfitting and overfitting so this can be considered as good fit.
